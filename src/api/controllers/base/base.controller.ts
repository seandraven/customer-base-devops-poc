...
import {
  ...
  SystemError,
  ValidationError,
} from "../../../infra/error/validation.error";
...
export class BaseController {
  async executionAPI<T>(
    ...
    try {
      ...
      logger.error(
        new LogData("Error transaction", {
          errors: ex.errors,
          response: ex.errors?.response?.data,
        }),
